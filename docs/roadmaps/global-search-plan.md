...
| **Code areas** | `data/search-aliases.json`, ranking weights, analytics review docs |
...
### Done for future V1 implementation (Phase C+)

- [ ] Search documents built from explore loaders + catalog + podcast + aliases  
- [ ] Client MiniSearch over cached index; no privileged keys  
- [ ] `/search` shareable, filterable, accessible  
- [ ] Quick search + shortcuts (Phase D)  
- [ ] Ranking fixtures green; gzip under alert budget  
- [ ] Analytics events consent-gated without raw query logging by default  
- [ ] Manuscript full text still excluded  
