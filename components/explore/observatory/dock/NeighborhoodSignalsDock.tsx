...
export function NeighborhoodSignalsDock({
  ...
  return (
    <div className={compact ? "space-y-2 text-xs text-muted" : "space-y-3 text-sm text-muted"}>
      {!compact ? <p className="font-display text-sm leading-relaxed text-fg">{signals.summaryLine}</p> : null}
        ...
        {signals.strongestEdges.map((e, i) => (
          <li key={`strong-${i}`}>
            ...
            >
              ...
              <span className="ml-2 text-fg">
                {formatRelationshipLabelForDisplay(e.relationship)}{" "}
                {labelFor(e.sourceId, index)} → {labelFor(e.targetId, index)}
              </span>
            </button>
          </li>
        ))}
          ...
          <li>
            ...
            <span className="ml-2 text-fg">
              {signals.bridgeNodeIds.slice(0, 4).map((id, i) => (
                <span key={id}>
                  <button type="button" className="hover:text-accent" onClick={() => onFocusNodeId?.(id)}>
